...
import { useSelector } from "react-redux";
import api from "./../../../config/api"; // Ensure this points to the file containing your axios instance
import {
  Users,
  GraduationCap,
  UserCheck,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard icon={School} label="Campuses" value={loading ? "..." : campuses.length} colorClass="text-blue-500" bgClass="bg-blue-500/10" trend="LIVE" isDark={isDark} />
          ...
          {/* Dynamic Campus Directory Table */}
          <div className={cardClass}>
            <SectionHeader title="Campus Directory" isDark={isDark} />
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className={`text-[10px] uppercase tracking-wider text-gray-400 border-b ${isDark ? 'border-white/5' : 'border-gray-50'}`}>
                      <th className="pb-3 font-semibold">Campus Name</th>
                      <th className="pb-3 font-semibold">Location</th>
                      <th className="pb-3 font-semibold">Admins</th>
                      <th className="pb-3 font-semibold text-center">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-transparent">
                    {campuses.map((campus) => (
                      <tr key={campus.id} className="group hover:bg-black/5 transition-colors">
                        <td className="py-4">
                          <div>
                            <p className={`text-sm font-bold ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{campus.name}</p>
                            <p className="text-[10px] text-gray-500 font-mono tracking-tighter uppercase">{campus.code}</p>
                          </div>
                        </td>
                        <td className="py-4">
                          <div className="flex items-center gap-1 text-gray-500">
                            <MapPin size={12} />
                            <span className="text-xs">{campus.city}{campus.state ? `, ${campus.state}` : ''}</span>
                          </div>
                        </td>
                        <td className="py-4">
                          {campus.campusAdmins.length > 0 ? (
                            <div className="text-[11px]">
                              <p className={isDark ? 'text-gray-300' : 'text-gray-700 font-medium'}>{campus.campusAdmins[0].name}</p>
                              <p className="text-gray-500">{campus.campusAdmins[0].email}</p>
                            </div>
                          ) : (
                        ...
                        <td className="py-4 text-center">
                          {campus.isActive ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-red-400 bg-red-400/10 px-2 py-1 rounded-full text-[10px] font-bold">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className={cardClass}>
            <div className="flex justify-between items-center mb-4">
                <span className={`font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>February 2026</span>
            ...
            <div className={`grid grid-cols-7 text-center text-xs gap-y-3 font-medium ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                {[...Array(28)].map((_, i) => (
                    <div key={i} className={`h-7 w-7 flex items-center justify-center rounded-full mx-auto cursor-pointer transition-all ${i+1 === 26 ? 'bg-blue-500 text-white font-bold shadow-lg' : isDark ? 'hover:bg-white/5' : 'hover:bg-gray-100'}`}>
                        {i+1}
                    </div>
                ))}
            </div>
          ...
          {/* Attendance Overview */}
          <div className={cardClass}>
             <SectionHeader title="Attendance Overview" isDark={isDark} />
             ...
             <div className="space-y-3">
                 {[
                     ...
                     { label: "Absent", val: "8%", color: "bg-orange-500" },
                     { label: "Late", val: "5%", color: "bg-purple-500" },
                 ].map((item, i) => (
