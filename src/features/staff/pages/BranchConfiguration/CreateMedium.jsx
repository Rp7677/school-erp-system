...
const MediumManagement = () => {
  ...
  // --- UI STATES ---
  const [confirmModal, setConfirmModal] = useState({ show: false, medium: null });
  const [showInfo, setShowInfo] = useState(false);

  const URL_PREFIX = "/api/mediums";
  ...
  const fetchMediums = async () => {
    setFetching(true);
    try {
      const res = await api.get(`${URL_PREFIX}/view-all-mediums`);
      setMediums(res.data || []);
    } catch (error) {
  ...
  // --- HANDLERS ---
  const triggerToggleConfirm = (medium) => {
    setConfirmModal({ show: true, medium });
  };

  const handleToggleStatus = async () => {
    const { medium } = confirmModal;
    setConfirmModal({ show: false, medium: null });
    setTogglingId(medium.id);

    const endpoint = medium.active
      ? `${URL_PREFIX}/deactivate-medium/${medium.id}`
      : `${URL_PREFIX}/activate-medium/${medium.id}`;

    try {
      await api.put(endpoint);
      toast.success(`${medium.name} status updated`);
      fetchMediums();
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`${URL_PREFIX}/create-medium`, formData);
      toast.success("Medium registered successfully");
          ...
          <div className={`w-full max-w-md rounded-3xl border p-8 shadow-2xl animate-in zoom-in-95 duration-200 ${theme.panel}`}>
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold opacity-50 uppercase tracking-widest">Security Authorization</p>
              </div>
            </div>
            ...
            <p className="text-sm font-medium opacity-80 mb-8 leading-relaxed">
              ...
              {confirmModal.medium?.active ? "DEACTIVATE" : "ACTIVATE"}</span> the medium 
              <span className="font-bold"> {confirmModal.medium?.name}</span>? This will restrict access for linked students.
            </p>

            <div className="flex gap-3">
              <button 
                onClick={() => setConfirmModal({ show: false, medium: null })}
              ...
              <button 
                ...
                className={`flex-1 h-12 rounded-xl font-black uppercase text-[10px] tracking-widest text-white shadow-lg transition-all active:scale-95 ${
                    confirmModal.medium?.active ? "bg-red-500 shadow-red-500/20" : "bg-green-500 shadow-green-500/20"
                }`}
              >
                Proceed
              </button>
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-heading font-black tracking-tighter uppercase">
              Medium <span className="text-primary">Control</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={`flex items-center gap-2 p-1.5 rounded-2xl border ${theme.panel}`}>
            <div className="px-5 py-2 border-r border-white/5 text-center">
              ...
              <p className="text-sm font-black">{mediums.length}</p>
            </div>
            <div className="px-5 py-2 text-center">
              <p className="text-[8px] font-black uppercase opacity-40 leading-none mb-1">Active</p>
              <p className="text-sm font-black text-green-500">{mediums.filter((m) => m.active).length}</p>
            </div>
          </div>
          {view === "list" && (
            <button
          ...
          {view === "list" ? (
            <div className={`rounded-3xl border shadow-sm overflow-hidden ${theme.panel}`}>
              <div className="p-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 bg-primary/5">
                <h3 className="font-black uppercase text-title-table tracking-widest">Medium Registry</h3>
                    ...
                    {fetching ? (
                      ...
                      <tr>
                        <td colSpan={4} className="px-8 py-20 text-center opacity-30 font-black uppercase text-[10px] tracking-widest">
                          No Medium records encrypted
                        </td>
                      </tr>
                    ) : (
                      filteredMediums.map((medium) => (
                        <tr key={medium.id} className={`transition-colors ${theme.rowHover}`}>
                          ...
                          <td className="px-8 py-6">
                            <span className="text-small-table uppercase tracking-tight">{medium.name}</span>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-2">
                              <div className={`w-1.5 h-1.5 rounded-full ${medium.active ? "bg-green-500 animate-pulse" : "bg-red-500"}`} />
                              <span className={`text-small-table px-3 py-1 rounded-full border ${medium.active ? "border-green-500/30 text-green-500 bg-green-500/5" : "border-red-500/30 text-red-500 bg-red-500/5"}`}>
                          ...
                          <td className="px-8 py-6 text-right">
                            <button
                              onClick={() => triggerToggleConfirm(medium)}
                              disabled={togglingId === medium.id}
                              className={`inline-flex items-center justify-center w-10 h-10 rounded-xl transition-all active:scale-90 shadow-sm ${
                                medium.active ? "bg-red-500 text-white shadow-red-500/20" : "bg-green-500 text-white shadow-green-500/20"
                              }`}
                            >
                              {togglingId === medium.id ? <Loader2 size={16} className="animate-spin" /> : <Power size={18} />}
                            </button>
                          </td>
                        </tr>
                      ))
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              >
                <ArrowDownLeft className="rotate-90" size={14} /> Return to Terminal
              </button>
              ...
              <div className={`max-w-2xl mx-auto rounded-3xl border shadow-sm overflow-hidden ${theme.panel}`}>
                <div className="p-8 border-b border-white/5 bg-primary/5">
                  <h2 className="text-title-table font-black uppercase tracking-tighter">Initialize Medium</h2>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                  <div className="space-y-2">
                    <label className="text-heding-table font-black uppercase tracking-widest text-slate-500 ml-1 flex items-center gap-2">
                      <Hash size={12} className="text-primary" /> Medium Code
                    </label>
                    <input
                      type="text"
                      required
                      placeholder="E.G. ENG, GUJ, HIN"
                  ...
                  <div className="space-y-2">
                    <label className="text-heding-table font-black uppercase tracking-widest text-slate-500 ml-1 flex items-center gap-2">
                      <Type size={12} className="text-primary" /> Medium Label
                    </label>
                    <input
                      type="text"
                      required
          ...
          <aside className="lg:col-span-4 animate-in slide-in-from-right-4 duration-500">
            <div className={`rounded-3xl border p-8 sticky top-10 ${theme.panel}`}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="font-black uppercase text-xs tracking-widest">Protocol Info</h4>
              </div>

              <div className="space-y-6">
                <div>
                  <h5 className="text-[10px] font-black uppercase text-primary mb-2">System Mediums</h5>
                  <p className="text-xs leading-relaxed opacity-60">
                    Mediums define the language of instruction. Changing these values affects registration forms and student filtering across the terminal.
                  </p>
                </div>

                <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10">
                  <h5 className="text-[10px] font-black uppercase text-primary mb-2 flex items-center gap-2">
                    <CheckCircle2 size={12} /> Integrity Rules
                  </h5>
                  <p className="text-[10px] leading-relaxed opacity-70">
                    <span className="text-primary font-bold">CODES:</span> Must be unique 3-4 letter identifiers.<br/>
                    <span className="text-primary font-bold">STATUS:</span> Inactivating a medium prevents new enrollments in that language.
                  </p>
                </div>

                <ul className="space-y-3">
                    {['Capitalized Short Codes', 'Descriptive Full Names', 'Confirm all status toggles'].map((item, i) => (
