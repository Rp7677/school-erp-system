...
const FeeDueDatesManager = () => {
  ...
  useEffect(() => {
    const loadBasics = async () => {
      try {
        const [camp, ay, adm] = await Promise.all([
  ...
  useEffect(() => {
    if (filters.campusId) {
      ...
      // Fetch structures but we will filter them in the UI based on branchGradeId
  ...
  const fetchExistingSchedule = async () => {
    const { branchGradeId, branchGradeStreamId, academicYearId, admissionTypeId, feeStructureTypeId } = filters;
    ...
    setFetching(true);
    try {
      const query = `branchGradeId=${branchGradeId}&branchGradeStreamId=${branchGradeStreamId}&academicYearId=${academicYearId}&admissionTypeId=${admissionTypeId}&feeStructureTypeId=${feeStructureTypeId}`;
  ...
  const handleFetchInstallments = async () => {
    ...
    try {
      ...
      const installments = res.data.lines?.[0]?.installments;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className={`lg:col-span-4 p-6 rounded-[2.5rem] border ${isDark ? "bg-[#141414] border-white/5" : "bg-white border-gray-100 shadow-sm"} space-y-4 h-fit`}>
          ...
          <select value={filters.campusId} onChange={e => setFilters({...filters, campusId: e.target.value})} className={inputClass}>
            ...
            {data.campuses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
