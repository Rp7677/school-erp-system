...
import {
  ...
  List, LayoutGrid, ChevronLeft, ChevronRight, Building2,
  RefreshCw,
} from "lucide-react";
...
export const ViewCampusRole = () => {
  ...
  const [selectedCampusId, setSelectedCampusId] = useState(Number(localStorage.getItem("selectedCampusId")) || "");
  ...
  const fetchRoles = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/api/campuses/${selectedCampusId}/roles`);
  ...
  useEffect(() => { if (selectedCampusId) fetchRoles(); }, [selectedCampusId]);
  ...
  const handleViewPermissions = async (role) => {
    ...
    setRolePermissions([]);
    try {
      const response = await api.get(`/api/campuses/${selectedCampusId}/roles/${role.id}/permissions`);
  ...
  const handleToggleStatus = async (role) => {
    ...
    try {
      ...
      await api.patch(`/api/campuses/${selectedCampusId}/roles/${role.id}/status`, { active });
  ...
  const filteredRoles = useMemo(() => {
    ...
    return roles.filter((r) => {
      ...
      return name.includes(lowerTerm) || code.includes(lowerTerm);
    });
  }, [roles, searchTerm]);
  ...
  const CampusID = campuses.find((campus) => campus.campusId === selectedCampusId);
      ...
      <div className={`pb-4 mb-6 backdrop-blur-md transition-colors ${isDark ? 'bg-[#0F0F0F]/80' : 'bg-white/80'}`}>
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center py-4 gap-4">
          <div>
            ...
            <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
              Manage access levels and control system functionality
            </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigate('/staff/campusroles/createcampusroles')} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-white text-[11px] font-black uppercase tracking-wider hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 active:scale-95">
              <Plus size={14} /> Create Role
            </button>
          </div>
        </div>

        {/* Campus + Search + Page size */}
        <div className={`${card} p-5 mb-6`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {/* Campus Selection */}
            <div>
              <label className={label}>Select Campus</label>
              <div className="relative">
                <select
                  value={selectedCampusId}
                  onChange={(e) => { const id = Number(e.target.value); setSelectedCampusId(id); localStorage.setItem("selectedCampusId", id); }}
                  className={`${input} pr-8 appearance-none`}
                >
                  <option value="">Select Campus</option>
                  {campuses?.map((campus) => <option key={campus.campusId} value={campus.campusId}>{campus.campusName}</option>)}
                </select>
                <ChevronRight size={12} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none opacity-40 rotate-90" />
              </div>
            </div>
            {/* Search */}
            <div>
      ...
      {!selectedCampusId ? (
        <div className={`${card} p-16 text-center`}>
          <div className={`w-16 h-16 rounded-3xl mx-auto mb-4 flex items-center justify-center ${isDark ? 'bg-white/5' : 'bg-gray-100'}`}>
            <Shield size={28} className={isDark ? 'text-gray-600' : 'text-gray-400'} />
          </div>
          <p className={`text-sm font-bold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Select a campus to view roles</p>
          <p className={`text-xs mt-1 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>Please select a campus from the dropdown above</p>
        </div>
      ) : loading ? (
              ...
              {paginatedRoles.map((role) => (
                <div key={role.id} className={`${card} hover:-translate-y-1 hover:shadow-lg hover:border-primary/20`}>
                  <div className={`px-5 py-4 border-b flex items-center gap-3 ${isDark ? 'border-white/5' : 'border-gray-100'}`}>
                    ...
                    <div>
                      <p className={`font-bold text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>{role.name || "Unnamed Role"}</p>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Access control for {role.name?.toLowerCase() || "role"}.</p>
                    <div className="grid grid-cols-4 gap-2">
                      ...
                      <button onClick={() => navigate(`/staff/campusroles/editcampusrole/${selectedCampusId}/${role.id}`)} className={`col-span-1 flex items-center justify-center rounded-xl border transition-all ${isDark ? 'border-white/10 text-gray-400 hover:bg-white/5' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}><Edit2 size={14} /></button>
                    </div>
                  </div>
                </div>
              ))}
                        ...
                        <tr key={role.id} className={trHover}>
                          ...
                          <td className={`${tdCell} text-center`}>
                            <div className="flex justify-center gap-1.5">
                              ...
                              <button onClick={() => navigate(`/staff/campusroles/editcampusrole/${selectedCampusId}/${role.id}`)} className={`p-2 rounded-xl border transition-all ${isDark ? 'border-white/10 text-gray-400 hover:bg-white/5' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`} title="Edit"><Edit2 size={14} /></button>
